...
function messageFilter(client, message) {
	...
	for (let blacklisted_index = 0; blacklisted_index < blacklisted_links.length; blacklisted_index += 1) {
		...
		let get_type = client.functions.getTranslation(client, message.author, message.guild, "events/message_filter", "generic_type");
		switch (blacklisted_links[blacklisted_index].type) {
			case "gif_crasher": { get_type = client.functions.getTranslation(client, message.author, message.guild, "events/message_filter", "gif_crasher_type"); break; }
			case "video_crasher": { get_type = client.functions.getTranslation(client, message.author, message.guild, "events/message_filter", "video_crasher_type"); break; }
