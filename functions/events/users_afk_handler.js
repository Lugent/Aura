...
async function messageAFK(client, message) {
	...
	if (get_afk) {
		...
		let afk_message = client.functions.getTranslation(client, message.author, message.guild, "afk_handler", "caller.returned", [message.author.tag]) + "\n" + client.functions.getTranslation(client, message.author, message.guild, "afk_handler", "caller.time", [afk_time]);
		...
		return message.inlineReply(afk_message).then(async (sent_message) => {
			sent_message.delete({timeout: 3000});
		});
	}
	...
	for (let index = 0; index < users.length; index++) {
		...
		if (get_user) {
			...
			let afk_message = client.functions.getTranslation(client, message.author, message.guild, "afk_handler", "pinged", [message.author.tag]) + "\n" + client.functions.getTranslation(client, message.author, message.guild, "afk_handler", "pinged.time", [afk_time]) + "\n\n" + get_user.reason;
